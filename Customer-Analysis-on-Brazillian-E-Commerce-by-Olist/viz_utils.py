...
class AnnotateBars:
    ...
    color: str = "black"
    n_dec: int = 2
    def horizontal(self, ax: Axis, centered:bool=False, orde:str='units'):
        ...
        self._annotate(ax, get_vals, ha=ha, va="center", orde=orde)
    def vertical(self, ax: Axis, centered:bool=False, orde:str='units'):
        def get_vals(p: Patch) -> PosVal:
            value = p.get_height()
            div = 2 if centered else 1
        ...
        self._annotate(ax, get_vals, ha="center", va=va, orde=orde)
    def _annotate(self, ax, func: PosValFunc, orde, **kwargs):
        cfg = {"color": self.color,
               "fontsize": self.font_size, **kwargs}
        for p in ax.patches:
            value, pos = func(p)
            if orde == 'thousands':
                ax.annotate(f"{value/1000:.{self.n_dec}f} K", pos, **cfg)
            else:
...
def single_countplot(df, ax, x=None, y=None, top=None, order=True, hue=False, palette='plasma',
    ...
    if x:
        ...
        for p in ax.patches:
            ...
            ax.annotate('{:.2f}%'.format(100. * x / ncount), (x, y.mean()), va='center')
...
def catplot_analysis(df_categorical, fig_cols=3, hue=False, palette='viridis', figsize=(16, 10)):
    ...
    for col in cat_features:
        ...
        format_spines(ax, right_border=False)
        AnnotateBars(n_dec=0, color='dimgrey').horizontal(ax,orde='units')
        ax.set_title(col)
...
def mean_sum_analysis(df, group_col, value_col, orient='vertical', palette='plasma', figsize=(15, 6)):
    ...
    if orient == 'vertical':
        ...
        AnnotateBars(n_dec=0, font_size=12, color='black').horizontal(axs[0],orde='units')
        AnnotateBars(n_dec=2, font_size=12, color='black').horizontal(axs[1],orde='thousands')
        ...
        AnnotateBars(n_dec=0, font_size=12, color='black').vertical(axs[0],orde='units')
        AnnotateBars(n_dec=2, font_size=12, color='black').vertical(axs[1],orde='thousands')

    # Customizing plot
    for ax in axs:
        format_spines(ax, right_border=False)
        ax.set_xlabel('{}'.format(' '.join(value_col.split('_')).title()))
        ax.set_ylabel('')
    axs[0].set_title('Mean {} by {}'.format(' '.join(value_col.split('_')).title(),
                                            ' '.join(group_col.split('_')).title()),
                                            size=14, color='dimgrey')
    axs[1].set_title('{} Amount by {}'.format(' '.join(value_col.split('_')).title(),
                                              ' '.join(group_col.split('_')).title()), 
                                              size=14, color='dimgrey')
